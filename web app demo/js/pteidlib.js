...
var libpteid = ffi.Library('libpteid', {
...
initPTEID = function(){
	codeCompare(libpteid.PTEID_Init("")); // Inicia
};

setSODChecking = function(value){
	codeCompare(libpteid.PTEID_SetSODChecking(value)); // Desabilita as verificações
};
...
getCardType = function(){
	...
	switch(cardType){
		case 0 : return("Error with the card type (pteid.CARD_TYPE_ERR)"); break;
		case 1 : return("IAS07"); break;
		case 2 : return("IAS101"); break;
	};
};
...
getPicture = function(){
	...
	var picFile = "pages/images/photo.jp2"
	var jpegPic = "pages/images/photo.jpg"

	fs.open(picFile, "w+", function(err, fd){
		fs.write(fd, picturefinal.picture.buffer, 0, picturefinal.picture.buffer.length, 'binary', function(err) {
            if (err) throw 'error writing file: ' + err;
            fs.close(fd, function(err) {
                if(err)
                    console.log(err);
                var cp = require('child_process');
    			var gm = cp.spawn('gm', ['convert', picFile, jpegPic]);
            })
        });  
    })
};
...
getID = function(){
	...
	var data = [ 
		idfinal.deliveryEntity.buffer.toString(),
		idfinal.country.buffer.toString(),
		idfinal.documentType.buffer.toString(),
		idfinal.cardNumber.buffer.toString(),
		idfinal.cardNumberPAN.buffer.toString(), 
		idfinal.cardVersion.buffer.toString(),
		idfinal.deliveryDate.buffer.toString(),
		idfinal.locale.buffer.toString(),
		idfinal.validityDate.buffer.toString(),
		idfinal.name.buffer.toString(),
		idfinal.firstname.buffer.toString(),
		idfinal.sex.buffer.toString(),
		idfinal.nationality.buffer.toString(),
		idfinal.birthday.buffer.toString(),
		idfinal.height.buffer.toString(),
		idfinal.numBI.buffer.toString(),
		idfinal.nameFather.buffer.toString(),
		idfinal.firstnameFather.buffer.toString(),
		idfinal.nameMother.buffer.toString(),
		idfinal.firstnameMother.buffer.toString(),
		idfinal.numNIF.buffer.toString(),
		idfinal.numSS.buffer.toString(),
		idfinal.numSNS.buffer.toString(),
		idfinal.notes.buffer.toString()
	]
...
getCertificates = function(){
	...
	var data = [];
	for(var i = 0; i < (certfinal.certificates.length); i++)
		data.push(certfinal.certificates[i].certifLabel.buffer.toString());

	return JSON.stringify(data);
};

module.exports = function(app, db) {
  app.get('/getCartao', (req, res) => {
    initPTEID();
    setSODChecking(false);
    var cardType = getCardType();
    var data = JSON.parse(getID())
    var certifs = JSON.parse(getCertificates());
    getPicture();

    var deliveryEntity = data[0];
    var country = data[1];
    var documentType = data[2];
    var cardNumber = data[3];
    var cardNumberPAN = data[4];
    var cardVersion = data[5];
	var deliveryDate = data[6];
	var locale = data[7];
	var validityDate = data[8];
	var name = data[9];
	var firstName = data[10];
	var sex = data[11];
	var nationality = data[12];
	var birthday = data[13];
	var height = data[14];
	var numBI = data[15];
	var nameFather = data[16];
	var firstnameFather = data[17];
	var nameMother = data[18];
	var firstnameMother = data[19];
	var numNIF = data[20];
	var numSS = data[21];
	var numSNS = data[22];
	var notes = data[23];
